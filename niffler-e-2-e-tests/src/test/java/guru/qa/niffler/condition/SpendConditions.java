        ...
        return new WebElementsCondition() {
            ...
            public CheckResult check(Driver driver, List<WebElement> elements) {
                ...
                if (expectedSpends.length != elements.size()) {
                    errorMessage = String.format("List size mismatch (expected: %s, actual: %s)", expectedSpends.length, elements.size());
                ...
                if (!passed) {
                    return rejected(errorMessage, actualSpendsList.stream().toList());
                }
                return accepted();
            }
